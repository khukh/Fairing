...
double CyAlPas(double mach, double alpha, double Hg) {
	...
	if (mach < 0.8) {
		if (alpha < 0) { 
			Cy = 0.190594304652482 - 6.38406136886791*a1 - 33.5164265635582*a2 - 30.704636866701*a3 - 10.9159604526799*a4 - 1.3746778158162*a5;
		}
		else {
			Cy = 0.413481820060867 + 4.71103549294835*a1 - 1.76350613168867*a2;
		}
	} else {
		if (mach < 1.2) {
			if (alpha < 0) {
...
double CxPas(double mach, double alpha, double Hg) {
	...
	if (mach < 0.8) {
		if (alpha < 0) {
			Cx = 0.63926001519838 - 0.162277253027975*a1 - 7.77570070224025*a2 - 9.65679071603829*a3 - 4.97317651231006*a4 - 1.18095024713667*a5 - 0.105813731416767*a6;
		}
		else {
			Cx = 0.672129227883525 + 0.108693252293213*a1 + 0.906856987527982*a2 - 1.45582628050291*a3 + 0.633634965318283*a4 - 0.0900901742641017*a5;
		}
	}
	else {
		if (mach < 1.2) {
...
double MzAlphaPas(double mach, double alpha, double Hg) {
	...
	if (mach < 0.8) {
		if (alpha < 0) {
			mz = 0.30779142651144 - 0.277854038854256*a1 - 6.65707437924101*a2 - 10.3248375924987*a3 - 6.78103257929276*a4 - 2.30964522348498*a5 - 0.401738062965339*a6 - 0.0279362656081643*a7;
		}
		else {
			mz = 0.344548567530313 + 0.562705635856*a1 - 0.851458363904556*a2 + 0.610883020728777*a3 - 0.5142261300719*a4 + 0.2117342289834*a5 - 0.028417459543695*a6;
		}
	}
	else {
		if (mach < 1.2) {
